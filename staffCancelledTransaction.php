...
$coop = new CoopServer();
$user = $coop->home();
$cancelled = $coop->getCancelledOrders();
                ...
                <div class="h-full">
                    <div class="grid grid-cols-2">
                        ...
                        <div class="justify-end pr-2 w-full flex items-center">
                            ...
                            <a href="staffCancelledTransaction.php"><p class="text-xs ml-3 text-[#2986CC] font-bold outline outline-offset-1 outline-[#221E3F] rounded-md bg-[#43365E] p-2 text-white">CANCELLED</p></a>
                            ...
                            <div class="justify-self-start w-full bg-[#ffd695] rounded-b-lg rounded-t-lg">
                                <div class="overflow-auto h-[32rem] max-h-106">
                                    <table class="justify-self-stretch w-full m-auto">
                                        ...
                                        <?php if ($cancelled) { ?>
                                            <?php foreach($cancelled as $cancelledOrders): ?>
                                                <tr class="cursor-pointer hover:bg-[#e8d2ae]">
                                                    <td class="py-2 px-3"><p><?= $cancelledOrders['transaction_id']; ?></p></td>
                                                    <td><p><?= $cancelledOrders['date']; ?></p></td>
                                                    <td><p><?= $cancelledOrders['fname']." ".$cancelledOrders['lname']; ?></p></td>
                                                    <td><p><?= $cancelledOrders['name']; ?></p></td>
                                                    <td><p><?= $cancelledOrders['process']; ?></p></td>
                                                    <td><p><?= $cancelledOrders['state']; ?></p></td>
                                                    <td><p><?= $cancelledOrders['amount']; ?></p></td>
                                                    <td class="py-2 px-3"><a href="staffViewOrder.php?transactionID=<?= $cancelledOrders['transaction_id']?>"><button class="ml-1 rounded-lg bg-[#221E3F] px-4 text-white hover:bg-[#6257b4] text-white p-2 text-sm">View</button></a></td>
                                                </tr>
                                            <?php endforeach; ?>
                                    <?php } ?>
                                    </table>
